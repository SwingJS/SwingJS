...
String PositionenString, LoesungenString;
  ...
  //  BufferedReader in = new BufferedReader ( new FileReader(file) );
    BufferedReader in = new BufferedReader ( new InputStreamReader ( (new URL(getCodeBase(), Dateiname)).openStream(), "8859_1" ) );
    do {}
...
int merkeFi = -1;
  ...
  if (img==null) {
    ...
    for (int i=0; i<(BilderAnz); i++) {
      ...
      try
      {
	    Bilder[i] = javax.imageio.ImageIO.read(new java.net.URL(getCodeBase(), (String)(BilderNamen[i])));
	  }
	  catch (Exception e) {}
  ...
  for (int i=0; i<(maleF.dimX); i++) {
    for (int j=0; j<(maleF.dimY); j++) {
      if (maleF.Elem[i][j] == -1) {
        imggc.fill3DRect(maleF.UrspX+i*(maleF.BildW+maleF.Rand),
