...
//
//changes made:
//		- DiffractionCanvas.paint() --> DiffractionCanvas.paintComponent()
//		- DiffractionFrame.paint() --> DiffractionFrame.paintComponent()
...
class DiffractionFrame extends Frame
  implements ComponentListener, ActionListener, AdjustmentListener,
	...
	add(gridBar = new Scrollbar(Scrollbar.HORIZONTAL, 250, 2, 10, 600));
	...
	Aperture createNext() { return new RingAperture(); }
	boolean hasXSymmetry() { return true; }
	boolean hasYSymmetry() { return true; }
	...
	String getName() { return "ring"; }
	Aperture createNext() { return new HalfCircleAperture(); }
	boolean hasXSymmetry() { return true; }
	boolean hasYSymmetry() { return true; }
	boolean hasDiagonalSymmetry() { return true; }
	void setToDefaults() { radius1 = .15; radius2 = .25; }
	void rezoom(double z) { radius1 *= z; radius2 *= z; }
	void compute() {
	    int i, j;
	    for (i = 0; i != gridSizeX/2; i++) {
		for (j = 0; j <= i; j++) {
	...
	double getDimension() { return radius2*2; }
    }

    class HalfCircleAperture extends Aperture {
	double radius;
	String getName() { return "half circle"; }
	...
	void drawGeometricShadow(Graphics g) {
		...
		g.setColor(Color.yellow);
	    int r = (int) (winSize.width*radius);
	    g.drawArc(winSize.width/2-r, winSize.height/2-r,
		      r*2, r*2, -90, 180);
	    g.drawLine(winSize.width/2, winSize.height/2-r,
		       winSize.width/2, winSize.height/2+r);
	}
