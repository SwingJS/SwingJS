...
//fT2b  2D  - 2T2g
	static double CalcD2T2g(double x1) 	{
		return -0.0000007532 * x1*x1*x1*x1*x1 + 0.00009886 * x1*x1*x1*x1 - 0.004352 * x1*x1*x1 + 0.06092 * x1*x1 + 1.378 * x1 + 24.8951929;
	}
    ...
    if(ratioFound){
      for(x1 = (x - 1); x1 < x; x1 += 0.02){
        ...
        if(temp_ratio21 < ratio21){
          ...
          String Bprime= (tmp.substring(0,tmp.length()-1) + "." + tmp.substring(tmp.length()-1,tmp.length()));
          y3 = CalcP4T1g(x1);
...
class TSd3Canvas extends JPanel implements MouseListener {
  ...
  boolean mouseOn;
  double x1,y1,y2,y3,ratio21,ratio31,ratio32,fE,fT2,fT1,fA1,fT2b,fT2H,fT1H,fEH;
  ...
  public void redraw(int range)   {
    switch (range)     {
      case 0:start_x=0;end_x=50;lrange=0;
      break;
      case 1:start_x=0;end_x=10;lrange=1;
      break;
      case 2:start_x=10;end_x=20;lrange=2;
      break;
      case 3:start_x=20;end_x=30;lrange=3;
      break;
      case 4:start_x=30;end_x=40;lrange=4;
      break;
      case 5:start_x=40;end_x=50;lrange=4;
      break;
    }
    repaint();
  }
  ...
  private void displayGraph(Graphics g)   {
	...
	Stroke stroke_d1 = new BasicStroke(1,BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL, 0, new float[] { 6, 6 }, 0);
...
//    Rectangle outPRec = new Rectangle(450,10,495,210);
    ...
    g2.drawLine(bottomLeft.x, bottomLeft.y, bottomLeft.x+graphRec.width, bottomLeft.y);
    ...
    else yDivisor = TanSugd3S.CalcP4T1g(end_x);
...
// define vlines so that the Y axis is marked every 10 or 5 units
      ...
      g2.drawLine(bottomLeft.x, bottomLeft.y-(ii * hSlines), bottomLeft.x+graphRec.width, bottomLeft.y-(ii * hSlines) );
  	}
	  for (int ii = 1; ii <= vlines; ii++)     {
	    g2.drawLine(bottomLeft.x+(ii * vSlines), bottomLeft.y, bottomLeft.x+(ii * vSlines), bottomLeft.y-graphRec.height);
  	}
...
//System.out.println(deltaB+","+bottomLeft.x);
      ...
      g2.setFont(f);
      g2.drawString("delta/B'   "+toThreePlaces(deltaB),outPRec.x,outPRec.y+15);
      g2.setColor(Color.red);
      g2.drawString("v1(T2g)/B' "+toThreePlaces(y1),outPRec.x,outPRec.y+28);
      g2.setColor(Color.blue);
      g2.drawString("v2(T1g)/B' "+toThreePlaces(y2),outPRec.x,outPRec.y+43);
      g2.setColor(dkgreen);
      g2.drawString("v3(T1g)/B' "+toThreePlaces(y3),outPRec.x,outPRec.y+56);
      g2.setColor(Color.black);
      g2.drawString("ratio v2/v1 "+toThreePlaces(ratio21),outPRec.x,outPRec.y+70);
      g2.setColor(teal);
      g2.drawString("f1 (Eg)/B' "+toThreePlaces(fE),outPRec.x,outPRec.y+83);
      g2.setColor(Color.orange);
      g2.drawString("f2(T1g)/B' "+toThreePlaces(fT1),outPRec.x,outPRec.y+98);
      g2.setColor(gold);
      g2.drawString("f3(T2g)/B' "+toThreePlaces(fT2),outPRec.x,outPRec.y+113);
      g2.setColor(Color.gray);
      g2.drawString("f4(A1g)/B' "+toThreePlaces(fA1),outPRec.x,outPRec.y+128);
      g2.setColor(lgreen);
      g2.drawString("f5(T2g)/B' "+toThreePlaces(fT2H),outPRec.x,outPRec.y+143);
      g2.setColor(copper);
      g2.drawString("f6(Eg)/B'  "+toThreePlaces(fEH),outPRec.x,outPRec.y+159);
      g2.setColor(dgreen);
      g2.drawString("f7(T1g)/B' "+toThreePlaces(fT1H),outPRec.x,outPRec.y+173);
      g2.setColor(purple);
      g2.drawString("f8(T2g)/B' "+toThreePlaces(fT2b),outPRec.x,outPRec.y+189);
      mouseOn = false;
    }
  }
  ...
  private void drawXtoY(double x, double y, int [] screen_x, int [] screen_y, Graphics g)   {
    int end_x1,end_y1;
