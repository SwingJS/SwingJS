...
import jssun.misc.MessageUtils;
import jssun.misc.Queue;

/**
 * Applet panel class. The panel manages and manipulates the applet as it is
 * being loaded. It forks a separate thread in a new thread group to call the
 * applet's init(), start(), stop(), and destroy() methods.
 * 
 * @author Arthur van Hoff
 */
public abstract class AppletPanel extends Panel implements AppletStub {
	 ...
	 * 
	 * Note: (XXX) is the action APPLET_XXX is the state (applet code loaded) -->
	 * APPLET_LOAD -- (applet init called)--> APPLET_INIT -- ( applet start
	 * called) --> APPLET_START -- (applet stop called) -->APPLET_STOP --(applet
	 * destroyed called) --> APPLET_DESTROY -->(applet gets disposed) -->
	 * APPLET_DISPOSE -->....
	 * 
	 * In the legacy lifecycle model. The applet gets loaded, inited and started.
	 * So it stays in the APPLET_START state unless the applet goes away(refresh
	 * page or leave the page). So the applet stop method called and the applet
	 * enters APPLET_STOP state. Then if the applet is revisited, it will call
	 * applet start method and enter the APPLET_START state and stay there.
	 * 
	 * In the modern lifecycle model. When the applet first time visited, it is
	 * same as legacy lifecycle model. However, when the applet page goes away. It
	 * calls applet stop method and enters APPLET_STOP state and then applet
	 * destroyed method gets called and enters APPLET_DESTROY state.
	 * 
	 * This code is also called by AppletViewer. In AppletViewer "Restart" menu,
	 * the applet is jump from APPLET_STOP to APPLET_DESTROY and to APPLET_INIT .
	 * 
	 * Also, the applet can jump from APPLET_INIT state to APPLET_DESTROY (in
	 * Netscape/Mozilla case). Same as APPLET_LOAD to APPLET_DISPOSE since all of
	 * this are triggered by browser.
	 * 
	 * 
	 */
	public void run(AppletEvent evt) {
		...
		try {
			...
			case APPLET_STOP:
				...
				// During Applet.stop(), any AccessControlException on an involved Class
				// remains in
				// the "memory" of the AppletClassLoader. If the same instance of the
				// ClassLoader is
			...
			case APPLET_DESTROY:
				...
				// During Applet.destroy(), any AccessControlException on an involved
				// Class remains in
				// the "memory" of the AppletClassLoader. If the same instance of the
				// ClassLoader is
				// reused, the same exception will occur during class loading. Set the
	...
	// try {
	// meth =
	// KeyboardFocusManager.class.getDeclaredMethod("getMostRecentFocusOwner", new
	// Class[] {Window.class});
...
//		}
	...
	/**
	 * Status line. Called by the AppletPanel to provide feedback on the Applet's
	 * state.
	 */
	...
	/**
	 * Called by the AppletPanel to provide feedback when an exception has
	 * happened.
	 */
	...
	public String getClassLoaderCacheKey() {
		/**
		 * Fixed #4501142: Classlaoder sharing policy doesn't take "archive" into
		 * account. This will be overridden by Java Plug-in. [stanleyh]
		 */
	...
	// public static void changeFrameAppContext(Frame frame, AppContext
	// newAppContext)
	// {
	...
	// (Vector<Window>)oldAppContext.get(Window.class);
	// if (windowList != null) {
	// for (Window ref : windowList) {
	// if (ref == frame) {
	// weakRef = ref;
	// break;
	// }
	...
	// {
	// Vector<Window> windowList = (Vector)newAppContext.get(Window.class);
	// if (windowList == null) {
	...
	// AppletMessageHandler("appletpanel");
}
