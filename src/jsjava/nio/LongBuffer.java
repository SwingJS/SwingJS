 ...
 * {@link #put(long) </code><i>put</i><code>} methods that read and write single
 * longs;
 * </p>
 * </li>
 * 
 * <li>
 * <p>
 * Relative {@link #get(long[]) </code><i>bulk get</i><code>} methods that
 * transfer contiguous sequences of longs from this buffer into an array; and
 * </p>
 * </li>
 * 
 * <li>
 * <p>
 * Relative {@link #put(long[]) </code><i>bulk put</i><code>} methods that
 * transfer contiguous sequences of longs from a long array or some other long
 ...
 * </code>duplicating<code>}, and {@link #slice </code>slicing<code>} a long
 * buffer.
 * </p>
 * </li>
 * 
 ...
 * long array into a buffer, or by creating a <a
 ...
 * href="ByteBuffer.html#direct"><i>direct</i> or <i>non-direct</i></a>. A long
...
public abstract class LongBuffer extends Buffer implements
	...
	LongBuffer(int mark, int pos, int lim, int cap, // package-private
		...
		this.hb = (hb == null ? new long[cap] : hb);
		this.offset = offset;
	}
	...
	public static LongBuffer allocate(int capacity) {
		...
		return new HeapLongBuffer(capacity, capacity, capacity, capacity);
	}
	 ...
	 * the number of longs remaining in this buffer, and its mark will be
	...
	/**
	 * Absolute <i>get</i> method. Reads the long at the given index. </p>
	 * 
	 * @param index
	 ...
	 * </pre>
	 * 
	 * except that it first checks that there are sufficient longs in this buffer
	 ...
	 * 
	 * @param offset
	 *          The offset within the array of the first long to be written; must
	 *          be non-negative and no larger than <tt>dst.length</tt>
	 * 
	 * @param length
	 *          The maximum number of longs to be written to the given array; must
	 *          be non-negative and no larger than <tt>dst.length - offset</tt>
	 * 
	 * @return This buffer
	 * 
	 * @throws BufferUnderflowException
	 *           If there are fewer than <tt>length</tt> longs remaining in this
	 *           buffer
	 * 
	 * @throws IndexOutOfBoundsException
	 *           If the preconditions on the <tt>offset</tt> and <tt>length</tt>
	 ...
	 * this buffer, that is, if <tt>src.remaining()</tt>&nbsp;<tt>&gt;</tt>&nbsp;
	 * <tt>remaining()</tt>, then no longs are transferred and a
	 ...
	 * 
	 * @param offset
	 *          The offset within the array of the first long to be read; must be
	 ...
	 * 
	 * <p>
	 * The longs between the buffer's current position and its limit, if any, are
	 * copied to the beginning of the buffer. That is, the long at index
	 * <i>p</i>&nbsp;=&nbsp;<tt>position()</tt> is copied to index zero, the long
	 * at index <i>p</i>&nbsp;+&nbsp;1 is copied to index one, and so forth until
	 * the long at index <tt>limit()</tt>&nbsp;-&nbsp;1 is copied to index
	 ...
	 * 
	 * <p>
	 * The buffer's position is set to the number of longs copied, rather than to
	 ...
	 * existing <tt>long</tt> array is the {@link ByteOrder#nativeOrder
	 * </code>native order<code>} of the underlying hardware. The byte order of a
	 * long buffer created as a <a href="ByteBuffer.html#views">view</a> of a byte
