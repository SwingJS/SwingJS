...
Clazz.load (["sun.util.calendar.AbstractCalendar", "$.CalendarDate"], "sun.util.calendar.BaseCalendar", ["java.lang.IllegalArgumentException", "sun.util.calendar.CalendarUtils"], function () {
c$ = Clazz.declareType (sun.util.calendar, "BaseCalendar", sun.util.calendar.AbstractCalendar);
...
if (dow != sun.util.calendar.CalendarDate.FIELD_UNDEFINED && dow != this.getDayOfWeek (bdate)) {
return false;
...
Clazz.defineMethod (c$, "getMonthLength", 
 function (year, month) {
var days = sun.util.calendar.BaseCalendar.DAYS_IN_MONTH[month];
...
return dayOfMonth + (this.isLeapYear (year) ? sun.util.calendar.BaseCalendar.ACCUMULATED_DAYS_IN_MONTH_LEAP[month] : sun.util.calendar.BaseCalendar.ACCUMULATED_DAYS_IN_MONTH[month]);
}, "~N,~N,~N");
...
if (n >= 0 && n < sun.util.calendar.BaseCalendar.FIXED_DATES.length) {
var jan1 = sun.util.calendar.BaseCalendar.FIXED_DATES[n];
if (cache != null) {
...
days += (365 * prevyear) + sun.util.calendar.CalendarUtils.floorDivide (prevyear, 4) - sun.util.calendar.CalendarUtils.floorDivide (prevyear, 100) + sun.util.calendar.CalendarUtils.floorDivide (prevyear, 400) + sun.util.calendar.CalendarUtils.floorDivide ((367 * month - 362), 12);
}if (month > 2) {
days -= this.isLeapYear (year) ? 1 : 2;
}if (cache != null && isJan1) {
cache.setCache (year, days, this.isLeapYear (year) ? 366 : 365);
}return days;
}, "~N,~N,~N,sun.util.calendar.BaseCalendar.Date");
...
month = sun.util.calendar.CalendarUtils.floorDivide (month, 367);
}var month1 = jan1 + sun.util.calendar.BaseCalendar.ACCUMULATED_DAYS_IN_MONTH[month];
if (isLeap && month >= 3) {
++month1;
}var dayOfMonth = (fixedDate - month1) + 1;
var dayOfWeek = sun.util.calendar.BaseCalendar.getDayOfWeekFromFixedDate (fixedDate);
...
return sun.util.calendar.BaseCalendar.getDayOfWeekFromFixedDate (fixedDate);
}, "sun.util.calendar.CalendarDate");
c$.getDayOfWeekFromFixedDate = Clazz.defineMethod (c$, "getDayOfWeekFromFixedDate", 
function (fixedDate) {
if (fixedDate >= 0) {
return (fixedDate % 7) + 1;
}return sun.util.calendar.CalendarUtils.mod (fixedDate, 7) + 1;
}, "~N");
...
d0 = fixedDate - 1;
n400 = sun.util.calendar.CalendarUtils.floorDivide (d0, 146097);
d1 = sun.util.calendar.CalendarUtils.mod (d0, 146097);
n100 = sun.util.calendar.CalendarUtils.floorDivide (d1, 36524);
d2 = sun.util.calendar.CalendarUtils.mod (d1, 36524);
n4 = sun.util.calendar.CalendarUtils.floorDivide (d2, 1461);
d3 = sun.util.calendar.CalendarUtils.mod (d2, 1461);
n1 = sun.util.calendar.CalendarUtils.floorDivide (d3, 365);
d4 = sun.util.calendar.CalendarUtils.mod (d3, 365) + 1;
