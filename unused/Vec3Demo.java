...
class Vec3DemoFrame extends Frame implements ComponentListener, ActionListener,
	...
	public void init() {
		...
		VecFunction vf = new E(InverseSquaredRadial(), InverseRotational(),
		...
		/*
		 * infoButton = new Button("Function Info"); add(infoButton);
		...
		for (i = 0; i != functionList.size(); i++)
			functionChooser.add(((VecFunction) functionList.elementAt(i)).getName());
		add(functionChooser);
		...
		add(partCountBar = new Scrollbar(Scrollbar.HORIZONTAL, 500, 1, 1,
				maxParticleCount));
	...
	void positionParticle(Particle p) {
		...
		p.pos[1] = besty * densitygroupsize + getrand(100) * densitygroupsize
				/ 100.0 - 1;
		p.pos[2] = bestz * densitygroupsize + getrand(100) * densitygroupsize
				/ 100.0 - 1;
	...
	void unmap3d(double x3[], int x, int y, double pn[], double pp[],
		...
		double t = ((pp[0] - cameraPos[0]) * pn[0] + (pp[1] - cameraPos[1]) * pn[1] + (pp[2] - cameraPos[2])
				* pn[2])
	...
	public void updateVec3Demo(Graphics realg) {
		...
		if (!stoppedCheck.getState()) {
			...
			if (disp != DISP_VECTORS && disp != DISP_VECTORS_A && disp != DISP_LINES
					&& disp != DISP_EQUIPS) {
				moveParticles();
				allquiet = false;
			}
			currentStep = (int) (reverse * (lastTime / 30) % 800);
			if (currentStep < 0)
				currentStep += 800;
		} else {
			lastXRot = lastYRot = 0;
...
//			// #ifndef BUILD_M
	...
	Color getCurrentColor(int i) {
		return (((currentStep / 2 + 400 - i) & 4) > 0) ? Color.yellow
				: Color.darkGray;
	}
		...
		for (i = 0; i != ct; i++) {
			...
			for (j = 0; j != ct; j++) {
				...
				if (d > 0) {
					...
					map3d(xoff + sinth1 * cosph2, sinth1 * sinph2, costh1, xpoints,
							ypoints, 1);
					map3d(xoff + sinth2 * cosph2, sinth2 * sinph2, costh2, xpoints,
							ypoints, 2);
					map3d(xoff + sinth2 * cosph1, sinth2 * sinph1, costh2, xpoints,
							ypoints, 3);
	...
	void drawVectors(Graphics g) {
		...
		if (!sliced) {
			vectorSpacing /= 2;
			if (vectors == null)
				vectors = new FieldVector[vectorSpacing * vectorSpacing * vectorSpacing];
	...
	void genLines() {
		...
		if (sliced)
			lineGridSize *= 2;
		int ct = (sliced) ? 30 * lineGridSize * lineGridSize : 30 * lineGridSize
				* lineGridSize * lineGridSize;
		vectors = new FieldVector[ct];
		double brightmult = 160 * strengthBar.getValue();

		boolean lineGrid[][][] = new boolean[lineGridSize][lineGridSize][lineGridSize];
	...
	void tryEdges(EquipPoint ep1, EquipPoint ep2, EquipPoint ep3, EquipPoint ep4) {
	...
	void interpPoint(EquipPoint ep1, EquipPoint ep2, double pval, double pos[]) {
	...
	void tryEdge(EquipPoint ep1, EquipPoint ep2, EquipPoint ep3, EquipPoint ep4) {
		int i;
	...
	void moveParticles() {
		...
		boolean withforce = (dispChooser.getSelectedIndex() == DISP_PART_FORCE);
		int maxd = (10 * getParticleCount() / (densitygridsize * densitygridsize * densitygridsize));
		if (sliced)
	...
	void renderItems(Graphics g, int pri) {
		...
		for (i = 0; i < pcount; i++) {
			...
			if (disp == DISP_PART_MAG) {
				...
				map3d(pos[0] + rhatx, pos[1] + rhaty, pos[2] + rhatz, xpoints, ypoints,
						1);
				drawArrow(g, null, xpoints[0], ypoints[0], xpoints[1], ypoints[1], 2);
			} else
	...
	boolean backFacing(double px, double py, double pz, double nx, double ny,
			double nz) {
	...
	int intersectSphere(double cp[], double ptx, double pty, double ptz, double r) {
	...
	double intersectZPlane(double cp[], double a, double ptx, double pty,
			double ptz) {
		double vx = ptx - cp[0];
		double vy = pty - cp[1];
		double vz = ptz - cp[2];
	...
	void rotateParticleAdd(double result[], double y[], double mult, double cx,
			double cy) {
		...
		if (e.getSource() == infoButton) {
			...
			try {
				...
				applet.getAppletContext().showDocument(
						new URL(applet.getCodeBase(), "functions.html" + '#' + s),
						"functionHelp");
			} catch (Exception ex) {
			}
		}
	...
	void dispChooserChanged() {
		...
		vecDensityLabel.setText(disp == DISP_VIEW_PAPER ? "Resolution"
				: "Vector Density");
		if (disp == DISP_EQUIPS) {
			strengthLabel.hide();
			strengthBar.hide();
	...
	void rk(int order, double x, double Y[], double stepsize) {
		...
		if (order == 3) {
			...
			for (i = 0; i != order; i++)
				Y[i] = Y[i] + fmult * (rk_k1[i] + 2 * (rk_k2[i] + rk_k3[i]) + rk_k4[i])
						/ 6;
		} else {
	...
	void getForceField(double result[], double y[], double stepsize, double fmult) {
	...
	void lineSegment(Particle p, int dir) {
		...
		while (true) {
			...
			if (Y[0] < -1 || Y[0] >= .999 || Y[1] < -1 || Y[1] >= .999 || Y[2] < -1
					|| Y[2] >= .999) {
				for (i = 0; i != order; i++)
					Y[i] = Yhalf[i] = oldY[i];
				h /= 2;
		...
		p.phi = java.lang.Math.sqrt(ls_fieldavg[0] * ls_fieldavg[0]
				+ ls_fieldavg[1] * ls_fieldavg[1] + ls_fieldavg[2] * ls_fieldavg[2])
				/ avgct;
	}
	...
	class InverseSquaredRadial extends VecFunction {
		String getName() {
			return E("point charge", null, "1/r^2 single");
		}
		...
		void drawCharge(Graphics g, double x, double y, double z, int dir) {
			...
			if (dir != 0)
				drawArrow(g, null, xpoints[0], ypoints[0], xpoints[1], ypoints[1], 5);
		}
	...
	class InverseSquaredRadialDouble extends InverseSquaredRadial {
		String getName() {
			return E("point charge double", null, "1/r^2 double");
		}
		...
		int getViewPri(double cameraPos[], double x[]) {
			double sep = aux1Bar.getValue() / 100.;
			if (intersectSphere(cameraPos, x[0], x[1], x[2], +sep, 0, 0, chargeSize) == 0
					&& intersectSphere(cameraPos, x[0], x[1], x[2], -sep, 0, 0,
							chargeSize) == 0)
				return 1;
			return 0;
		}
		...
		VecFunction createNext() {
			return E(new InverseSquaredRadialDipole(), null, new InverseRadial());
		}
	};
	...
	class InverseSquaredRadialQuad extends InverseSquaredRadial {
		...
		int getViewPri(double cameraPos[], double x[]) {
			...
			for (i = -1; i <= 1; i += 2)
				for (j = -1; j <= 1; j += 2)
					if (intersectSphere(cameraPos, x[0], x[1], x[2], i * sep, j * sep, 0,
							.06) != 0)
						return 0;
			return 1;
		}
	...
	class InverseRadial extends VecFunction {
		...
		String getName() {
			return E("charged line", null, "1/r single line");
		}
		...
		int getViewPri(double cameraPos[], double x[]) {
			if (intersectCylinder(cameraPos, x[0], x[1], x[2], lineWidth, true) == 0)
				return 1;
	...
	class InverseRadialDouble extends VecFunction {
		...
		String getName() {
			return E("line charge double", null, "1/r double lines");
		}
		...
		void getField(double result[], double y[]) {
			...
			if (r1 < lineWidth || r2 < lineWidth)
				boundCheck = true;
			if (getPot) {
				result[0] = .2 * (java.lang.Math.log(r1 + 1e-20) + sign
		...
		int getViewPri(double cameraPos[], double x[]) {
			...
			double sep = aux1Bar.getValue() / 100.;
			for (i = -1; i <= 1; i += 2) {
				if (intersectCylinder(cameraPos, x[0], x[1], x[2], i * sep, 0,
						lineWidth, true) != 0)
					return 0;
			}
			return 1;
		...
		VecFunction createNext() {
			return E(new InverseRadialDipole(), null, new InverseRotational());
		}
	};
	...
	class InverseRadialQuad extends VecFunction {
		...
		void getField(double result[], double y[]) {
			...
			if (r1 < lineWidth || r2 < lineWidth || r3 < lineWidth || r4 < lineWidth)
				boundCheck = true;
			if (getPot) {
				result[0] = .2 * (+java.lang.Math.log(r1 + 1e-20)
						- java.lang.Math.log(r2 + 1e-20) - java.lang.Math.log(r3 + 1e-20) + java.lang.Math
						.log(r4 + 1e-20));
				return;
			}
			double q = .0003;
		...
		int getViewPri(double cameraPos[], double x[]) {
			...
			for (i = -1; i <= 1; i += 2) {
				for (j = -1; j <= 1; j += 2) {
					if (intersectCylinder(cameraPos, x[0], x[1], x[2], i * sep, j * sep,
							.01, true) != 0)
						return 0;
				}
			}
	...
	class FiniteChargedLine extends InverseRadial {
		...
		void getLineField(double result[], double y[], double off) {
			...
			double fth = q
					* (-1 / (a12 + y2 + a1 * a12y2) + 1 / (a22 + y2 + a2 * a22y2));
	...
	class ChargedPlate extends ConductingPlate {
		...
		double getPot(double a1, double a2, double y) {
			...
			cz.log();
			double y2 = y * y;
			return .2 * (2 * (a1 - a2) + (b1 - cz.b) * y + a2
					* java.lang.Math.log(a2 * a2 + y2) - a1
					* java.lang.Math.log(a1 * a1 + y2));
		}

		void getField(double result[], double y[]) {
			if (y[2] >= -.01 && y[2] <= .01 && (y[0] >= -a && y[0] <= a))
				boundCheck = true;
			double a1 = -a - y[0];
			double a2 = a - y[0];
			double y2 = y[2] * y[2];
			...
			double q = .0003 / a;
			result[0] = .5 * q * java.lang.Math.log((y2 + a2 * a2) / (y2 + a1 * a1));
			result[1] = 0;
			result[2] = q
					* (java.lang.Math.atan(a1 / y[2]) - java.lang.Math.atan(a2 / y[2]));
		}
	...
	class ChargedPlatePair extends ChargedPlate {
		...
		void getField(double result[], double y[]) {
			double sep = aux2Bar.getValue() / 100.;
			if ((y[2] >= -.01 + sep && y[2] <= .01 + sep || y[2] >= -.01 - sep
					&& y[2] <= .01 - sep)
					&& y[0] >= -a && y[0] <= a)
				boundCheck = true;
			double a1 = -a - y[0];
			...
			if (y22 == 0)
				y22 = 1e-8;
			if (getPot) {
				result[0] = getPot(a1, a2, y1) - getPot(a1, a2, y2);
				return;
			}
			...
			result[0] = .5
					* q
					* (java.lang.Math.log((y12 + a2 * a2) / (y12 + a1 * a1)) - java.lang.Math
							.log((y22 + a2 * a2) / (y22 + a1 * a1)));
			result[1] = 0;
			result[2] = q
					* (java.lang.Math.atan(a1 / y1) - java.lang.Math.atan(a2 / y1)
							- java.lang.Math.atan(a1 / y2) + java.lang.Math.atan(a2 / y2));
		}
	...
	class SphereAndPointCharge extends InverseSquaredRadial {
		...
		double getSphereRadius() {
			return (aux1Bar.getValue() + 1) / 110.;
		}

		double getSeparation() {
			return aux2Bar.getValue() / 100.;
		}
		...
		double getPointPos() {
			return -getSeparation() / 2 - getSphereRadius();
		}
		...
		void render(Graphics g) {
			int ic = intersectSphere(cameraPos, getPointPos() - getSpherePos(), 0, 0,
					getSphereRadius());
			if (ic != 0)
		...
		int getViewPri(double cameraPos[], double x[]) {
			if (intersectSphere(cameraPos, x[0], x[1], x[2], getSpherePos(), 0, 0,
					getSphereRadius()) != 0)
				return -1;
			if (intersectSphere(cameraPos, x[0], x[1], x[2], getPointPos(), 0, 0,
					chargeSize) != 0)
				return -1;
			return 0;
		}
	...
	class CylinderAndLineCharge extends VecFunction {
		...
		void getField(double result[], double y[]) {
			...
			if (getPot) {
				result[0] = -700
						* (cq * java.lang.Math.log(r1 + 1e-20) - q
		...
		void render(Graphics g) {
			int ic = intersectCylinder(cameraPos, getPointPos(), 0, 0, getCylPos(),
					0, getCylRadius(), false);
			if (ic == 0)
		...
		int getViewPri(double cameraPos[], double x[]) {
			if (intersectCylinder(cameraPos, x[0], x[1], x[2], getPointPos(), 0,
					lineWidth, true) != 0)
				return -1;
			if (intersectCylinder(cameraPos, x[0], x[1], x[2], getCylPos(), 0,
					getCylRadius(), true) != 0)
				return -1;
			return 0;
		}
			...
			if (r < a) {
				...
				if (conducting)
					...
					if (getPot)
						...
						result[2] = (showD) ? e1 * fmult * (1 - dimult) : fmult
								* (1 - dimult);
				}
				return;
			}
	...
	class CylinderInField extends VecFunction {
		...
		int getViewPri(double cameraPos[], double x[]) {
			return intersectCylinder(cameraPos, x[0], x[1], x[2], a, conducting);
		}
	...
	class DielectricBoundaryE extends InverseSquaredRadial {
		...
		int getViewPri(double cameraPos[], double x[]) {
			double cx = aux1Bar.getValue() / 50. - 1.001;
			if (intersectSphere(cameraPos, x[0], x[1], x[2] - cx, chargeSize) == 0
	...
	class FastChargeEField extends MovingChargeField {
		...
		void getField(double result[], double y[]) {
			...
			double b = -.0001
					* (1 - beta * beta)
					/ (rz * rz * rz * java.lang.Math.pow(1 - beta * beta * sinth * sinth,
							1.5));
			result[0] = b * y[0];
			result[1] = b * y[1];
			result[2] = b * y[2];
	...
	// String getName() { return E(null, "current line",
	// "1/r rotational"); }
	// void setup() {
	// setXYViewExact();
	// }
	...
	// int dir2;
	// boolean ext;
	// String getName() { return E(null, "current line double",
	...
	// String getName() { return E(null, "cur line double + ext",
	// "1/r rot double + ext"); }
	...
	// String getName() { return E(null, "current line dipole",
	...
	class MovingChargeField extends InverseSquaredRadial {
		...
		VecFunction createNext() {
			return E(null, new FastChargeField(), null);
		}
	};
	...
	class CurrentLoopField extends VecFunction {
		...
		void getLoopField(double result[], double y[], double xoff, double zoff,
				int dir, double size) {
			double xx = y[0] - xoff;
			double yy = y[1];
			double zz = y[2] - zoff;
		...
		void renderLoop(Graphics g, double xoff, double zoff, int dir, double size) {
			...
			for (i = 0; i != loopSegments; i++) {
				...
				if (i == 0 && useColor)
					drawCurrentArrow(g, xpoints[0], ypoints[0], xpoints[1], ypoints[1]);
				else
					g.drawLine(xpoints[0], ypoints[0], xpoints[1], ypoints[1]);
			}
		}
		...
		VecFunction createNext() {
			return E(null, new CurrentLoopsSideField(), null);
		}
	};
	...
	class Complex {
		...
		void sin() {
			set(cosh(b) * java.lang.Math.sin(a), java.lang.Math.cos(a) * sinh(b));
		}

		void cos() {
			set(cosh(b) * java.lang.Math.cos(a), java.lang.Math.sin(a) * sinh(b));
		}
