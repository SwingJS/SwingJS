 ...
 * <li>
 * <p>
 * Relative {@link #get(short[]) </code><i>bulk get</i><code>} methods that
 * transfer contiguous sequences of shorts from this buffer into an array; and
 * </p>
 * </li>
 * 
 * <li>
 * <p>
 * Relative {@link #put(short[]) </code><i>bulk put</i><code>} methods that
 * transfer contiguous sequences of shorts from a short array or some other
 * short buffer into this buffer;&#32;and
 * </p>
 * </li>
 * 
 * 
 ...
 * </code>duplicating<code>}, and {@link #slice </code>slicing<code>} a short
 * buffer.
 * </p>
 * </li>
 * 
 ...
 * short array into a buffer, or by creating a <a
 ...
 * 
 * <p>
 * Like a byte buffer, a short buffer is either <a
 * href="ByteBuffer.html#direct"><i>direct</i> or <i>non-direct</i></a>. A short
 * buffer created via the <tt>wrap</tt> methods of this class will be
...
public abstract class ShortBuffer extends Buffer implements
	 ...
	 * the number of shorts remaining in this buffer, and its mark will be
	...
	/**
	 * Absolute <i>get</i> method. Reads the short at the given index. </p>
	 * 
	 * @param index
	 ...
	 * 
	 * <p>
	 * Otherwise, this method copies <tt>length</tt> shorts from this buffer into
	 * the given array, starting at the current position of this buffer and at the
	 * given offset in the array. The position of this buffer is then incremented
	 * by <tt>length</tt>.
	 * 
	 * <p>
	 ...
	 * </pre>
	 * 
	 * except that it first checks that there are sufficient shorts in this buffer
	 ...
	 * 
	 * @param offset
	 *          The offset within the array of the first short to be written; must
	 ...
	 * 
	 * @throws BufferUnderflowException
	 *           If there are fewer than <tt>length</tt> shorts remaining in this
	 *           buffer
	 * 
	 * @throws IndexOutOfBoundsException
	 *           If the preconditions on the <tt>offset</tt> and <tt>length</tt>
	 ...
	 * this buffer. If there are more shorts remaining in the source buffer than
	 ...
	 * 
	 * @param src
	 *          The source buffer from which shorts are to be read; must not be
	 *          this buffer
	 * 
	 * @return This buffer
	 * 
	 ...
	 * If there are more shorts to be copied from the array than remain in this
	 ...
	 * 
	 * @param offset
	 *          The offset within the array of the first short to be read; must be
	 *          non-negative and no larger than <tt>array.length</tt>
	 * 
	 * @param length
	 ...
	 * 
	 * <p>
	 * The shorts between the buffer's current position and its limit, if any, are
	 * copied to the beginning of the buffer. That is, the short at index
	 * <i>p</i>&nbsp;=&nbsp;<tt>position()</tt> is copied to index zero, the short
	 * at index <i>p</i>&nbsp;+&nbsp;1 is copied to index one, and so forth until
	 * the short at index <tt>limit()</tt>&nbsp;-&nbsp;1 is copied to index
	 ...
	 * 
	 * <p>
	 * The buffer's position is set to the number of shorts copied, rather than to
	 ...
	 * existing <tt>short</tt> array is the {@link ByteOrder#nativeOrder
	 * </code>native order<code>} of the underlying hardware. The byte order of a
	 * short buffer created as a <a href="ByteBuffer.html#views">view</a> of a
