 ...
 * {@link #put(byte) </code><i>put</i><code>} methods that read and write single
 * bytes;
 * </p>
 * </li>
 * 
 * <li>
 * <p>
 * Relative {@link #get(byte[]) </code><i>bulk get</i><code>} methods that
 ...
 * <li>
 * <p>
 * Relative {@link #put(byte[]) </code><i>bulk put</i><code>} methods that
 ...
 * <li>
 * <p>
 * Absolute and relative {@link #getChar() </code><i>get</i><code>} and
 * {@link #putChar(char) </code><i>put</i><code>} methods that read and write
 * values of other primitive types, translating them to and from sequences of
 ...
 * </code>duplicating<code>}, and {@link #slice </code>slicing<code>} a byte
 * buffer.
 * </p>
 * </li>
 * 
 ...
 * 
 * 
 * content, or by {@link #wrap(byte[]) </code><i>wrapping</i><code>} an existing
 ...
 * {@link java.nio.channels.FileChannel#map </code>mapping<code>} a region of a
 * file directly into memory. An implementation of the Java platform may
 * optionally support the creation of direct byte buffers from native code via
 * JNI. If an instance of one of these kinds of buffers refers to an
 ...
 * 
 * <p>
 * Corresponding methods are defined for the types <tt>char</tt>, <tt>short</tt>, <tt>int</tt>, <tt>long</tt>, and <tt>double</tt>. The index parameters of
...
//	 * its mark will be undefined. Whether or not it has a {@link #hasArray
...
//	}
	...
	/**
	 * Absolute <i>get</i> method. Reads the byte at the given index. </p>
	 * 
	 * @param index
	 ...
	 * 
	 * @param length
	 *          The maximum number of bytes to be written to the given array; must
	 *          be non-negative and no larger than <tt>dst.length - offset</tt>
	 * 
	 * @return This buffer
	 * 
	 * @throws BufferUnderflowException
	 *           If there are fewer than <tt>length</tt> bytes remaining in this
	 *           buffer
	 * 
	 * @throws IndexOutOfBoundsException
	 *           If the preconditions on the <tt>offset</tt> and <tt>length</tt>
	 ...
	 * <i>p</i>&nbsp;=&nbsp;<tt>position()</tt> is copied to index zero, the byte
	 * at index <i>p</i>&nbsp;+&nbsp;1 is copied to index one, and so forth until
	 * the byte at index <tt>limit()</tt>&nbsp;-&nbsp;1 is copied to index
	 ...
	 * 
	 * @param bo
	 *          The new byte order, either {@link ByteOrder#BIG_ENDIAN BIG_ENDIAN}
	 ...
	 * into a char value according to the current byte order, and then increments
	 * the position by two.
	 * </p>
	 * 
	...
	/**
	 * Relative <i>put</i> method for writing a char value&nbsp;&nbsp;<i>(optional
	 * operation)</i>.
	 * 
	 * <p>
	...
	/**
	 * Absolute <i>put</i> method for writing a char value&nbsp;&nbsp;<i>(optional
	 * operation)</i>.
	 * 
	 * <p>
	 * Writes two bytes containing the given char value, in the current byte
