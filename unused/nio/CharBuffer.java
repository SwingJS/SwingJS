 ...
 * <li>
 * <p>
 * Absolute and relative {@link #get() </code><i>get</i><code>} and
 * {@link #put(char) </code><i>put</i><code>} methods that read and write single
 * characters;
 * </p>
 * </li>
 * 
 * <li>
 * <p>
 * Relative {@link #get(char[]) </code><i>bulk get</i><code>} methods that
 * transfer contiguous sequences of characters from this buffer into an array;
 * and
 * </p>
 * </li>
 * 
 * <li>
 * <p>
 * Relative {@link #put(char[]) </code><i>bulk put</i><code>} methods that
 * transfer contiguous sequences of characters from a character
 * array,&#32;a&#32;string, or some other character buffer into this
 * buffer;&#32;and
 * </p>
 * </li>
 * 
 ...
 * <li>
 * <p>
 * Methods for {@link #compact </code>compacting<code>}, {@link #duplicate
 * </code>duplicating<code>}, and {@link #slice </code>slicing<code>} a
 * character buffer.
 * </p>
 * </li>
 * 
 ...
 * character array or&#32;string into a buffer, or by creating a <a
 ...
 * 
 * <p>
 * Like a byte buffer, a character buffer is either <a
 * href="ByteBuffer.html#direct"><i>direct</i> or <i>non-direct</i></a>. A
 * character buffer created via the <tt>wrap</tt> methods of this class will be
 * non-direct. A character buffer created as a view of a byte buffer will be
 * direct if, and only if, the byte buffer itself is direct. Whether or not a
 * character buffer is direct may be determined by invoking the
 * {@link #isDirect isDirect} method.
 * </p>
 * 
 * 
 * 
...
public abstract class CharBuffer extends Buffer implements
	 ...
	 * its mark will be undefined. It will have a {@link #array </code>backing
	 * array<code>}, and its {@link #arrayOffset </code>array offset<code>} will
	 * be zero.
	 * 
	 * @param capacity
	 ...
	 * 
	 * <p>
	 * The new buffer will be backed by the given character array; that is,
	 ...
	 * {@link #array </code>backing array<code>} will be the given array, and its
	 * {@link #arrayOffset </code>array offset<code>} will be zero.
	 * </p>
	 * 
	 * @param array
	 ...
	 *          larger than <tt>array.length</tt>. The new buffer's position will
	 *          be set to this value.
	 * 
	 * @param length
	 *          The length of the subarray to be used; must be non-negative and no
	 *          larger than <tt>array.length - offset</tt>. The new buffer's limit
	 *          will be set to <tt>offset + length</tt>.
	 * 
	 * @return The new character buffer
	 * 
	 ...
	 * 
	 * @param start
	 *          The index of the first character to be used; must be non-negative
	 *          and no larger than <tt>csq.length()</tt>. The new buffer's
	 *          position will be set to this value.
	 * 
	 * @param end
	 *          The index of the character following the last character to be
	 *          used; must be no smaller than <tt>start</tt> and no larger than
	 *          <tt>csq.length()</tt>. The new buffer's limit will be set to this
	 *          value.
	 * 
	 * @return The new character buffer
	 * 
	 ...
	 * the number of characters remaining in this buffer, and its mark will be
	 ...
	 * 
	 * @throws IndexOutOfBoundsException
	 *           If <tt>index</tt> is negative or not smaller than the buffer's
	 *           limit
	 */
	 ...
	 * <tt>&gt;</tt>&nbsp;<tt>remaining()</tt>, then no characters are transferred
	 ...
	 * 
	 * @param offset
	 *          The offset within the array of the first character to be written;
	 *          must be non-negative and no larger than <tt>dst.length</tt>
	 * 
	 * @param length
	 *          The maximum number of characters to be written to the given array;
	 ...
	 * 
	 * @throws BufferUnderflowException
	 *           If there are fewer than <tt>length</tt> characters remaining in
	 *           this buffer
	 * 
	 * @throws IndexOutOfBoundsException
	 *           If the preconditions on the <tt>offset</tt> and <tt>length</tt>
	 ...
	 * characters from the given buffer into this buffer, starting at each
	 * buffer's current position. The positions of both buffers are then
	 ...
	 * 
	 * @param src
	 *          The source buffer from which characters are to be read; must not
	 *          be this buffer
	 * 
	 * @return This buffer
	 * 
	 ...
	 * array. If there are more characters to be copied from the array than remain
	 ...
	 * 
	 * @param length
	 *          The number of characters to be read from the given array; must be
	 ...
	 *          must be non-negative and no larger than <tt>string.length()</tt>
	 * 
	 * @param end
	...
	/**
	 * Returns the character array that backs this buffer&nbsp;&nbsp;<i>(optional
	 * operation)</i>.
	 * 
	 * <p>
	 ...
	 * of the buffer&nbsp;&nbsp;<i>(optional operation)</i>.
	 * 
	 * <p>
	 ...
	 * 
	 * <p>
	 * The characters between the buffer's current position and its limit, if any,
	 * are copied to the beginning of the buffer. That is, the character at index
	 * <i>p</i>&nbsp;=&nbsp;<tt>position()</tt> is copied to index zero, the
	 * character at index <i>p</i>&nbsp;+&nbsp;1 is copied to index one, and so
	 * forth until the character at index <tt>limit()</tt>&nbsp;-&nbsp;1 is copied
	 * to index <i>n</i>&nbsp;=&nbsp;<tt>limit()</tt>&nbsp;-&nbsp;<tt>1</tt>
	 * &nbsp;-&nbsp;<i>p</i>. The buffer's position is then set to <i>n+1</i> and
...
//			if ((v1 != v1) && (v2 != v2)) // For float and double
	 ...
	 * <tt>limit()</tt>&nbsp;-&nbsp;1. Invoking this method does not change the
	 * buffer's position.
	 * </p>
	 * 
	 ...
	 * this buffer is mutable then modifications to one buffer will cause the
	 * other to be modified. The new buffer's capacity will be that of this
	 * buffer, its position will be <tt>position()</tt>&nbsp;+&nbsp;<tt>start</tt>
	 * , and its limit will be <tt>position()</tt>&nbsp;+&nbsp;<tt>end</tt>. The
	 ...
	 * 
	 * @throws IndexOutOfBoundsException
	 *           If the preconditions on <tt>start</tt> and <tt>end</tt> do not
	 *           hold
	 */
	@Override
	 ...
	 * </code>native order<code>} of the underlying hardware. The byte order of a
	 * character buffer created as a <a href="ByteBuffer.html#views">view</a> of a
	 * byte buffer is that of the byte buffer at the moment that the view is
	 * created.
	 * </p>
	 * 
