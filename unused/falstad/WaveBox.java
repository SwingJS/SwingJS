...
class WaveBoxFrame extends Frame
  implements ComponentListener, ActionListener, AdjustmentListener,
	...
	for (i = 0; i != gridSizeX; i++)
		...
		for (n = 0; n < maxn; n++) {
			...
			cost1 = 1; sint1 = 0;
			cost2 = shiftcos; sint2 = shiftsin;
			double a = setup.computePoint(xxi, yyi, zzi);
			cost1 = 0; sint1 = 1;
			cost2 = -shiftsin; sint2 = shiftcos;
	...
	int mxhalf;
	int mxlast;
	String getName() { return "point source"; }
	void select() { }
	void precompute() {
	    int x, y, z;
	    mxhalf = maxTerms/2;
	...
	Setup createNext() { return new PinholeSetup(); }
    };
	...
	Setup createNext() { return new TwoSourcesSetup(); }
    };
	...
	int mxlast;
	boolean dipole;
	String getName() { return "2 point sources"; }
	void select() {
	...
	double computePoint(int x, int y, int z) {
	    ...
	    return (datadzr[d1][z]+datadzr[d2][z]-
		    datadzr[d3][z]-datadzr[d4][z])*cost1 -
		(datadzi[d1][z]+datadzi[d2][z]-
		 datadzi[d3][z]-datadzi[d4][z])*sint1;
	}
	...
	Setup createNext() { return new TwoPinholesSetup(); }
    };
	...
	int mxhalf;
	int mxlast;
	String getName() { return "2 pinholes"; }
	void select() {
	    setupBar(0, "Source Separation", 30);
	    setupBar(1, "Phase Difference", 0);
	    setupBar(2, "Balance", 50);
	...
	double getPhaseShift() { return aux2Bar.getValue()*pi/50.; }
	void deselect() {
	    dataxy = null;
	    datadzr = datadzi = null;
	}
	...
	Setup createNext() { return new SingleLineSetup(); }
    };
	...
	double datai[][];
	int mxhalf;
	String getName() { return "single line source"; }
	void select() { }
	void precompute() {
	    int x, y, z;
	    mxhalf = maxTerms/2;
	...
	Setup createNext() { return new DoubleLineSetup(); }
    };
	...
	String getName() { return "2 line sources"; }
	void select() {
	    setupBar(0, "Source Separation", 30);
	    setupBar(1, "Phase Difference", 0);
	    setupBar(2, "Balance", 50);
	...
	double computePoint(int x, int y, int z) {
	    int nx = mxlast - x;
	    return w1mult*(datar[ x][y]*cost1-datai[ x][y]*sint1) +
		   w2mult*(datar[nx][y]*cost2-datai[nx][y]*sint2);
	}
	...
	double datai[][][];
	boolean noz;
	String getName() { return "2 plane waves"; }
	void select() {
