...
function (g) {
System.out.println ("paint " + (++this.paintCount));
this.pg.updateAtomViewer (g);
}, "java.awt.Graphics");
...
function (target) {
var barwidth = 0;
for (var i = target.getComponentCount (); --i >= 0; ) {
...
test.falstad.AtomViewer.ogf.initA ();
}, "~A");
...
function (g) {
Clazz.superCall (this, test.falstad.AtomViewer, "paint", [g]);
...
Clazz.instantialize (this, arguments);
}, test.falstad, "AtomViewerFrame", javax.swing.JFrame, [java.awt.event.ComponentListener, java.awt.event.ActionListener, java.awt.event.AdjustmentListener, java.awt.event.MouseMotionListener, java.awt.event.MouseListener, java.awt.event.ItemListener]);
...
function () {
this.sampleCount = 15;
for (var i = 0, n = this.samplesNums.length; i < n; i++) {
...
for (var i = 1, n = this.sampleCount; i < n; i += 2) {
this.sampleMult[i] = 4;
...
function (realg) {
System.out.println ("...updating..." + !this.stoppedCheck.getState ());
var g = null;
...
if (!this.finished) {
return;
}if (e.getItemSelectable () === this.cubicItem) {
this.setLValue ();
this.setupDisplay ();
this.orbitalChanged ();
this.cv.repaint (this.pause);
...
var nsam = this.samplesNums.length;
var i;
for (i = 0; i != nsam; i++) if (this.samplesItems[i] === e.getItemSelectable ()) break;

if (i != nsam) {
var j;
for (j = 0; j != nsam; j++) this.samplesItems[j].setState (i == j);
...
} else if (e.getItemSelectable () === this.cubicItem) {
this.setLValue ();
this.orbitalChanged ();
}this.cv.repaint (this.pause);
...
if (ev.getID () == 201) {
System.exit (0);
}Clazz.superCall (this, test.falstad.AtomViewerFrame, "processEvent", [ev]);
}, "java.awt.AWTEvent");
Clazz.defineMethod (c$, "destroyFrame", 
function () {
...
for (var j = 0, n = ab.altStateCount; j != n; j++) {
...
if (ab.n == 0 || ab.xAxis != xAxis) continue;
ab.convertBasisToDerived ();
for (var j = 0, a = ab.altStateCount; j != a; j++) {
var ds = ab.altStates[j];
ds.rotate (ang * ds.m);
}
}
