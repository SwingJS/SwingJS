...
Clazz.load (["java.util.Vector", "jsjavax.swing.event.EventListenerList"], "jsjavax.swing.MenuSelectionManager", ["jsjava.awt.event.MouseEvent", "jsjavax.swing.JComponent", "$.SwingUtilities", "jsjavax.swing.event.ChangeEvent", "$.ChangeListener", "jssun.awt.AppContext"], function () {
c$ = Clazz.decorateAsClass (function () {
this.selection = null;
this.changeEvent = null;
this.listenerList = null;
Clazz.instantialize (this, arguments);
}, jsjavax.swing, "MenuSelectionManager");
...
var msm = context.get (jsjavax.swing.MenuSelectionManager.MENU_SELECTION_MANAGER_KEY);
if (msm == null) {
msm =  new jsjavax.swing.MenuSelectionManager ();
context.put (jsjavax.swing.MenuSelectionManager.MENU_SELECTION_MANAGER_KEY, msm);
}return msm;
}});
...
if (Clazz.instanceOf (mc, jsjavax.swing.JComponent)) {
cWidth = (mc).getWidth ();
...
p.y = screenY;
jsjavax.swing.SwingUtilities.convertPointFromScreen (p, mc);
...
var exitEvent =  new jsjava.awt.event.MouseEvent (oldMC, 505, event.getWhen (), event.getModifiers (), p.x, p.y, event.getXOnScreen (), event.getYOnScreen (), event.getClickCount (), event.isPopupTrigger (), 0);
currentSelection[currentSelection.length - 1].processMouseEvent (exitEvent, path, this);
var enterEvent =  new jsjava.awt.event.MouseEvent (mc, 504, event.getWhen (), event.getModifiers (), p.x, p.y, event.getXOnScreen (), event.getYOnScreen (), event.getClickCount (), event.isPopupTrigger (), 0);
subElements[j].processMouseEvent (enterEvent, path, this);
}var mouseEvent =  new jsjava.awt.event.MouseEvent (mc, event.getID (), event.getWhen (), event.getModifiers (), p.x, p.y, event.getXOnScreen (), event.getYOnScreen (), event.getClickCount (), event.isPopupTrigger (), 0);
...
return null;
}, "jsjava.awt.Component,jsjava.awt.Point");
...
}, $fz.isPrivate = true, $fz), "jsjavax.swing.MenuElement,jsjava.awt.Component");
c$.MENU_SELECTION_MANAGER_KEY = c$.prototype.MENU_SELECTION_MANAGER_KEY =  new JavaObject ();
});
