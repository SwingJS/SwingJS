...
Clazz.load (["jsjava.math.RoundingMode"], "jsjava.text.DigitList", ["java.lang.ArithmeticException", "$.Double", "$.InternalError", "$.Long", "$.StringBuffer"], function () {
...
}, jsjava.text, "DigitList", null, Cloneable);
Clazz.prepareFields (c$, function () {
this.digits =  Clazz.newCharArray (19, '\0');
this.roundingMode = jsjava.math.RoundingMode.HALF_EVEN;
});
...
var max = jsjava.text.DigitList.LONG_MIN_REP[i];
...
exponent = jsjava.text.DigitList.parseInt (source, i, len);
break;
} else {
if (!nonZeroDigitSeen) {
...
case jsjava.math.RoundingMode.UP:
for (var i = maximumDigits; i < this.count; ++i) {
if (this.digits[i] != '0') {
return true;
}}
break;
case jsjava.math.RoundingMode.DOWN:
break;
case jsjava.math.RoundingMode.CEILING:
for (var i = maximumDigits; i < this.count; ++i) {
if (this.digits[i] != '0') {
return !this.isNegative;
}}
break;
case jsjava.math.RoundingMode.FLOOR:
for (var i = maximumDigits; i < this.count; ++i) {
if (this.digits[i] != '0') {
return this.isNegative;
}}
break;
case jsjava.math.RoundingMode.HALF_UP:
if (this.digits[maximumDigits] >= '5') {
return true;
}break;
case jsjava.math.RoundingMode.HALF_DOWN:
if (this.digits[maximumDigits] > '5') {
return true;
...
case jsjava.math.RoundingMode.HALF_EVEN:
if (this.digits[maximumDigits] > '5') {
return true;
} else if (this.digits[maximumDigits] == '5') {
for (var i = maximumDigits + 1; i < this.count; ++i) {
...
case jsjava.math.RoundingMode.UNNECESSARY:
for (var i = maximumDigits; i < this.count; ++i) {
if (this.digits[i] != '0') {
...
System.arraycopy (jsjava.text.DigitList.LONG_MIN_REP, 0, this.digits, 0, this.count);
} else {
...
if (!(Clazz.instanceOf (obj, jsjava.text.DigitList))) return false;
var other = obj;
...
function () {
try {
var other = Clazz.superCall (this, jsjava.text.DigitList, "clone", []);
...
if (this.digits[i] != jsjava.text.DigitList.LONG_MIN_REP[i]) return false;
}
return true;
}, $fz.isPrivate = true, $fz));
c$.parseInt = Clazz.defineMethod (c$, "parseInt", 
($fz = function (str, offset, strLen) {
var c;
var positive = true;
if ((c = str[offset]) == '-') {
positive = false;
