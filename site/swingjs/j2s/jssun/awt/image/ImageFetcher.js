...
Clazz.load (["java.lang.Thread", "$.StringBuffer"], ["jssun.awt.image.FetcherInfo", "$.ImageFetcher"], ["java.security.AccessController", "$.PrivilegedAction", "java.util.Vector", "jssun.awt.AppContext"], function () {
c$ = Clazz.declareType (jssun.awt.image, "ImageFetcher", Thread);
Clazz.makeConstructor (c$, 
($fz = function (threadGroup, index) {
Clazz.superConstructor (this, jssun.awt.image.ImageFetcher, [threadGroup, "Image Fetcher " + index]);
this.setDaemon (true);
}, $fz.isPrivate = true, $fz), "ThreadGroup,~N");
...
java.security.AccessController.doPrivileged (((Clazz.isClassDefined ("jssun.awt.image.ImageFetcher$1") ? 0 : jssun.awt.image.ImageFetcher.$ImageFetcher$1$ ()), Clazz.innerTypeInstance (jssun.awt.image.ImageFetcher$1, this, Clazz.cloneFinals ("info", info, "fetcherGroup", fetcherGroup))));
return;
}, $fz.isPrivate = true, $fz), "jssun.awt.image.FetcherInfo");
c$.$ImageFetcher$1$ = function () {
Clazz.pu$h ();
c$ = Clazz.declareAnonymous (jssun.awt.image, "ImageFetcher$1", null, java.security.PrivilegedAction);
Clazz.overrideMethod (c$, "run", 
function () {
for (var i = 0; i < this.f$.info.fetchers.length; i++) {
if (this.f$.info.fetchers[i] == null) {
this.f$.info.fetchers[i] =  new jssun.awt.image.ImageFetcher (this.f$.fetcherGroup, i);
this.f$.info.fetchers[i].start ();
this.f$.info.numFetchers++;
break;
}}
return null;
});
